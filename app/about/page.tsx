...
function HighlightIcon({ kind }: { kind: "HEART" | "BROWSER" | "PLANT" }) {
  ...
  if (kind === "HEART")
    return (
      <svg className="hl-icon" aria-hidden="true" viewBox="0 0 16 16">
        <g fill={C}>
...
const DIVIDER_PIXELS = Array.from({ length: 24 }, (_, i) => `divider-pixel-${i}`);
    ...
    <div className="about fade-in">
      ...
      <section className="about-hero">
        ...
        <div className="highlight-row">
          {HIGHLIGHTS.map((h) => (
            <div key={h.t} className={`highlight ${h.c}`}>
      ...
      <div className="about-divider reveal" aria-hidden="true">
        ...
        <div className="div-pixels">
          {DIVIDER_PIXELS.map((id, i) => (
            <span key={id} style={{ animationDelay: `${i * 80}ms` }}></span>
          ))}
        </div>
        <div className="div-bar"></div>
      </div>
